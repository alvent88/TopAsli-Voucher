...
export default function AdminDashboard() {
  ...
  // Test Email Webhook states
  const [testingEmailWebhook, setTestingEmailWebhook] = useState(false);
  const [webhookFrom, setWebhookFrom] = useState("noreply@uniplay.id");
  const [webhookSubject, setWebhookSubject] = useState("Voucher Purchase Confirmation");
  const [webhookBody, setWebhookBody] = useState("Thank you for your purchase!\n\nYour voucher code: RBLX-1234-ABCD-5678\n\nPlease redeem this code in the game.");
  ...
  const handleTestEmailWebhook = async () => {
    setTestingEmailWebhook(true);
    setWebhookResponse("");
    
    try {
      toast({
        title: "🧪 Testing Email Webhook",
        description: "Testing voucher code extraction...",
      });
      
      const result = await backend.email.testWebhook({
        from: webhookFrom,
        subject: webhookSubject,
        body: webhookBody,
      });
      
      setWebhookResponse(JSON.stringify(result, null, 2));
      
      if (result.success && result.extractedCode) {
        toast({
          title: "✅ Webhook Test Berhasil!",
          description: `Voucher code extracted: ${result.extractedCode}`,
        });
      } else {
        toast({
          title: "❌ Webhook Test Gagal",
          description: result.message || "Could not extract voucher code",
          variant: "destructive",
        });
      }
    } catch (error: any) {
      console.error("Test email webhook error:", error);
      setWebhookResponse(JSON.stringify({
        error: error.message || "Unknown error",
        details: error.toString(),
      }, null, 2));
      ...
      toast({
        title: "Error",
        description: error.message || "Gagal test email webhook",
        variant: "destructive",
      });
    } finally {
      setTestingEmailWebhook(false);
    }
  };
        ...
        <Card className="bg-slate-900 border-slate-800">
          <CardHeader>
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle className="text-white">Test Email Webhook</CardTitle>
                <p className="text-sm text-slate-400 mt-1">Test extract voucher code dari email</p>
              </div>
            </div>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="webhook-from" className="text-slate-300">From Email</Label>
              <Input
                id="webhook-from"
                value={webhookFrom}
                onChange={(e) => setWebhookFrom(e.target.value)}
                className="bg-slate-800 border-slate-700 text-white"
                placeholder="noreply@uniplay.id"
              />
            </div>
            
            <div className="space-y-2">
              <Label htmlFor="webhook-subject" className="text-slate-300">Subject</Label>
              <Input
                id="webhook-subject"
                value={webhookSubject}
                onChange={(e) => setWebhookSubject(e.target.value)}
                className="bg-slate-800 border-slate-700 text-white"
                placeholder="Voucher Purchase Confirmation"
              />
            </div>
            
            <div className="space-y-2">
              <Label htmlFor="webhook-body" className="text-slate-300">Email Body</Label>
              <Textarea
                id="webhook-body"
                value={webhookBody}
                onChange={(e) => setWebhookBody(e.target.value)}
                className="bg-slate-800 border-slate-700 text-white h-32"
                placeholder="Email body dengan voucher code..."
              />
            </div>
            
            <Button
              onClick={handleTestEmailWebhook}
              disabled={testingEmailWebhook}
              className="w-full bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700"
            >
              {testingEmailWebhook ? "Testing..." : "🧪 Test Extract Voucher"}
            </Button>

            <div className="space-y-2">
              <Label className="text-slate-300">Extraction Result</Label>
              <Textarea
                value={webhookResponse}
                readOnly
                className="bg-slate-800 border-slate-700 text-indigo-400 font-mono text-xs h-48 resize-none"
                placeholder="Hasil extraction akan muncul di sini..."
              />
            </div>

            <div className="text-xs text-slate-500 bg-slate-800/50 p-3 rounded-lg border border-slate-700">
              <strong className="text-slate-400">💡 Info:</strong> Test akan mencoba extract voucher code dari email body menggunakan berbagai pattern regex.
            </div>
            
            <div className="text-xs text-yellow-600 bg-yellow-900/20 p-3 rounded-lg border border-yellow-700">
              <strong className="text-yellow-500">📌 Webhook URL Production:</strong>
              <br />
              <code className="text-yellow-400 mt-1 block break-all">
                https://gaming-top-up-platform-d3pg4ec82vjikj791feg.api.lp.dev/email/webhook
              </code>
              <p className="mt-2 text-slate-400">Gunakan URL ini untuk setup Gmail Auto-Forward atau Email Forwarding Service</p>
            </div>
          </CardContent>
        </Card>
